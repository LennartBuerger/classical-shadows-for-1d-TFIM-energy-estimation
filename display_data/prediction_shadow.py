    ...
    for single_measurement in full_measurement:
        ...
        for pauli_XYZ, position in one_observable:
            if pauli_XYZ != single_measurement[position][0]:
                not_match = 1
                break
            product *= single_measurement[position][1] # here the outcome of the measurement is
...
if __name__ == "__main__":
    def print_usage():
        ...
        print("    We would output the predicted value for each local observable given in [observable.txt]",
              file=sys.stderr)
